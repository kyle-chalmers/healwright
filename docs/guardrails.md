...
| AI cannot push, or write outside its folder | The model has `Read`, `Grep`, `Glob`, and a path-scoped `Edit` (the allowed folder and `.healwright-out/summary.md`); `Bash`, `Write`, `WebFetch`, `WebSearch`, `NotebookEdit` and sub-agents are disallowed outright, not merely un-approved. The issue text and the folder's git history are handed to it as files. Structurally, no credential in that job can write to GitHub |
| Model output is data | The summary is redacted twice (best effort before upload, authoritatively in `publish` with the same shapes as `healwright_core.redact`, including high-entropy strings), size-capped, stripped of every backtick, and fenced as plain text; the artifact holds only the patch and the redacted summary and expires after one day |
