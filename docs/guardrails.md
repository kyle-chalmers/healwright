...
| AI cannot push, or write outside its folder | No shell `git`, `gh`, `Write`, `cat`, `tee`, `echo`, `find`, or `python` in the tool allowlist; `Edit` is path-scoped to the allowed folder and the summary file; the issue text and the folder's git history are handed to the model as files. Structurally, no credential in that job can write to GitHub |
| Model output is data | The agent's summary is redacted (secret and PII shapes), size-capped, and fenced as plain text before it reaches a PR body or issue comment |
